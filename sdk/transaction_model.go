...
func (tx *abstractTransaction) generateVectors(builder *flatbuffers.Builder) (v uint64, signatureV, signerV, dV, fV flatbuffers.UOffsetT, err error) {
	...
	dV = transactions.TransactionBufferCreateUint32Vector(builder, utils.FromBigInt(big.NewInt(tx.Deadline.GetInstant())))
...
func (tx *MosaicDefinitionTransaction) generateBytes() ([]byte, error) {
	...
	mV := transactions.TransactionBufferCreateUint32Vector(builder, utils.FromBigInt(tx.MosaicId.Id))
	dV := transactions.TransactionBufferCreateUint32Vector(builder, utils.FromBigInt(tx.MosaicProperties.Duration))
	nV := transactions.TransactionBufferCreateUint32Vector(builder, utils.FromBigInt(tx.NamespaceId.Id))
...
func (tx *MosaicSupplyChangeTransaction) generateBytes() ([]byte, error) {
	...
	mV := transactions.TransactionBufferCreateUint32Vector(builder, utils.FromBigInt(tx.MosaicId.Id))
	dV := transactions.TransactionBufferCreateUint32Vector(builder, utils.FromBigInt(tx.Delta))
...
func (tx *TransferTransaction) generateBytes() ([]byte, error) {
	...
	for i, mos := range tx.Mosaics {
		id := transactions.MosaicBufferCreateIdVector(builder, utils.FromBigInt(mos.MosaicId.Id))
		am := transactions.MosaicBufferCreateAmountVector(builder, utils.FromBigInt(mos.Amount))
...
func (tx *RegisterNamespaceTransaction) generateBytes() ([]byte, error) {
	...
	nV := transactions.TransactionBufferCreateUint32Vector(builder, utils.FromBigInt(tx.Id))
	var dV flatbuffers.UOffsetT
	if tx.NamespaceType == Root {
		dV = transactions.TransactionBufferCreateUint32Vector(builder, utils.FromBigInt(tx.Duration))
	} else {
		dV = transactions.TransactionBufferCreateUint32Vector(builder, utils.FromBigInt(tx.ParentId.Id))
	}
...
func (tx *LockFundsTransaction) generateBytes() ([]byte, error) {
	...
	mv := transactions.TransactionBufferCreateUint32Vector(builder, utils.FromBigInt(tx.Mosaic.MosaicId.Id))
	maV := transactions.TransactionBufferCreateUint32Vector(builder, utils.FromBigInt(tx.Mosaic.Amount))
	dV := transactions.TransactionBufferCreateUint32Vector(builder, utils.FromBigInt(tx.Duration))
